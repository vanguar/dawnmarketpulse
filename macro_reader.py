# macro_reader.py  –  макро-блок для 7 регионов с fallback на World Bank
...
LATEST_ROWS  = 6        # сколько точек грузим из FRED
...
def get_macro_block():
    ...
    for key, cfg in SERIES.items():
        ...
        # ── CPI YoY с fallback на World Bank ───────────────────
        try:
            if cfg["cpi_yoy"]:
